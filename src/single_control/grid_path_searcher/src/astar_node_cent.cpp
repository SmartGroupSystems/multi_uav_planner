...
Vector2d real_pos = Vector2d::Zero();
Vector2d _target_pt;
...
{   
    ...
    for (int idx = 0; idx < (int)cloud.points.size(); idx++)//遍历点云中的点
    {
        ...
        if(std::abs(pt.z-actual_height)>_resolution) continue;
         _astar_path_finder->setObs(pt.x - real_pos[0], 
                                    pt.y - real_pos[1]);
        // for visualize only
        // 可视化
        Vector2d cor_round = _astar_path_finder->coordRounding(Vector2d(pt.x - real_pos[0], 
                                                                        pt.y - real_pos[1]));
        pt.x = cor_round(0);
        pt.y = cor_round(1);
        pt.z = 0;
        cloud_vis.points.push_back(pt);
        Vector2i center_index = _astar_path_finder -> coord2gridIndex(Vector2d(pt.x - real_pos[0], 
                                                                               pt.y - real_pos[1]));
        ...
        for(int row_index = -1;row_index<=1;row_index++){
            for(int col_index = -1;col_index<=1;col_index++){
                ...
                if(_astar_path_finder -> getData(pt_around_i)) {
                    pt.x = pt_around_d(0) + real_pos[0];
                    pt.y = pt_around_d(1) + real_pos[1];
                    cloud_vis.points.push_back(pt);
                }
            }
        }
    ...
    cout << "point cloud size:"<<endl;
    cout << cloud_vis.width <<endl;
    ...
    }else  
    {
        ...
        {
            if(_astar_path_finder->getData(Eigen::Vector2d(_start_pt - real_pos))) 
            {
                // TODO 修改起点推出条件
                cout << "\033[33m[Astar] _start_pt is Occupied! Reset Obs. BE CAREFUL!\033[0m" << endl;
                _astar_path_finder -> cleanStartObs(Eigen::Vector2d(_start_pt - real_pos));
                // if(1)
                // {
            ...
            if(_astar_path_finder -> getData(Eigen::Vector2d(_start_pt - real_pos))) // 目标点被占据
            {
                std::pair<bool,Eigen::Vector2i> camecame_need;
                camecame_need = _astar_path_finder -> check_collision(Eigen::Vector2d(_target_pt - real_pos), 
                                                                      Eigen::Vector2d(_start_pt - real_pos));
                // camecame_need = _astar_path_finder -> check_collision(_target_pt, _start_pt);// FIXME
                ...
                else
                {
                    ...
                    end_warn_flag = false;
                    _target_pt = Vector2d(_astar_path_finder -> gridIndex2coord(camecame_need.second)+real_pos);
                }
            } 
            else
            {
            ...
            {
                if(!pathFinding(Eigen::Vector2d(_start_pt - real_pos), 
                                Eigen::Vector2d(_target_pt - real_pos),
                                current_vel,
                                current_acc)) 
...
//sub
    ...
    _rpos_sub = nh.subscribe( "real_pose",1,realPoseCallback);
//pub
    ...
    _astar_path_finder  -> initGridMap(_resolution, _map_lower, _map_upper, _max_x_id, _max_y_id, _interval);
