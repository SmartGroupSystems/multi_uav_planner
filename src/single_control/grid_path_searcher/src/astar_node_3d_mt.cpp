...
double current_seq = 0, sight_radius, _interval,threshold;
Vector3d _start_pt(0.0,0.0,1.2),current_vel(0.0,0.0,0.0),current_acc(0.0,0.0,0.0),real_pos(0.0,0.0,0.0);
Vector3d _target_pt;
vector<Vector3d> target_list;
// Map size
...
bool pathFinding(const Vector3d start_pt, const vector<Vector3d> target_list_, 
...
ROS_WARN("[A*]{sucess}  Time in vis map is %f ms", (time_2 - time_1).toSec() * 1000. );
    ...
    {
        // // 判断是否到达？
        // std::vector<Eigen::Vector3d> temp_target_list;
        // int final_target_index = 0;
        // for (size_t i = 0; i != target_list.size(); i++)
        // {
        //     auto target_ptr_ = target_list[i];
        //     if(_astar_path_finder->nearly_arrived(_start_pt,target_ptr_,threshold))
        //     {
        //         cout << "\033[32m[Astar] Arrived at point "<< target_ptr_ <<"! Target remove.\033[0m" << endl;
        //         final_target_index = i;
        //         continue;
        //     }
        //     // temp_target_list.push_back(target_ptr_);
        // }
        // // if(final_target_index != target_list.size()-1)
        // cout << final_target_index << endl;
        // for (int i = final_target_index; i < target_list.size(); i++)
        // {
        //     temp_target_list.push_back(target_list[i]);
        // }
        ...
        {
            cout << "\033[32m[Astar] All targets arrived!\n\033[31m[Astar] PathFinding will not run! \033[0m" << endl;
            _has_target = false;
        }
        else
        {
            bool end_warn_flag = false;
            ...
            {
                auto target_ptr_ = target_list[i];
                if(_astar_path_finder -> getData(target_ptr_)) // 目标点被占据
                {
                    std::pair<bool,Eigen::Vector3i> camecame_need;
                    camecame_need = _astar_path_finder -> check_collision(target_ptr_, _start_pt);
                    if(camecame_need.first) // 不存在可达点
                    {
                        cout << "\033[31m[Astar] target_ptr_ is Occupied!\033[0m" << endl;
                        end_warn_flag = true;
                        continue;
                    }
                    else
                    {
                        cout << "\033[33m[Astar] Get the nearest non-occupied target_ptr_.\033[0m" << endl;
                        target_ptr_ = _astar_path_finder -> gridIndex2coord(camecame_need.second);
                        temp_target_list.push_back(target_ptr_);
                    }
                }
                else
            ...
            if(!end_warn_flag && !temp_target_list.empty())
            {
                if(!pathFinding(_start_pt, temp_target_list,current_vel,current_acc)) 
...
{     
    ...
    _target_pt << wp->pose.position.x,
                  ...
                  1.5;
                //   wp->pose.position.z;
    target_list.push_back(_target_pt);
    _has_target = true;
...
void realPoseCallback(const geometry_msgs::PoseStamped & aim_msg)
{
    real_pos[0] = aim_msg.pose.position.x;
    real_pos[1] = aim_msg.pose.position.y;
    real_pos[2] = aim_msg.pose.position.z;
}
                 ...
                 const Vector3d vel_,     const Vector3d acc_)
{
    ...
    vector<Eigen::Vector3d> full_path;
    cout<<"[Astar] PathFinding start. Target_list_size: "<<target_list_.size()<<endl;
    auto start_pt_ = start_pt;
    for (size_t i = 0; i < target_list_.size(); i++)
    {
        auto target_pt_ = target_list_[i];
        cout<< "\033[46m[Astar] With start_pt_: "<<start_pt_<<" target_pt_: "<<target_pt_<<"\033[0m\n";
        ros::Time time_1 = ros::Time::now();
        if(!_astar_path_finder->AstarGraphSearch(start_pt_, target_pt_)) 
        {
            cout<< "no path provideeeeeeeeeeeeeeeeee" <<"\033[0m\n";
            return 0;
        }
        ros::Time time_2 = ros::Time::now();
        ROS_WARN("[A*]{sucess}  Time in GraphSearch for target %i is %f ms",i, (time_2 - time_1).toSec() * 1000. );
        
        //Retrieve the path
        time_1 = ros::Time::now();
        auto grid_twist    = _astar_path_finder->getTwist_checkcolision3(start_pt_);
        time_2 = ros::Time::now();
        ROS_WARN("[A*]{sucess}  Time in checkcolision for target %i  is %f ms",i, (time_2 - time_1).toSec() * 1000. );
        start_pt_ = target_list_[i];
        full_path.insert(full_path.end(),grid_twist.begin(),grid_twist.end());
        time_1 = ros::Time::now();
        _astar_path_finder->resetUsedGrids();
        time_2 = ros::Time::now();
        ROS_WARN("[A*]{sucess}  Time in resetUsedGrids for target %i  is %f ms",i, (time_2 - time_1).toSec() * 1000. );
    ...
    visGridPath (full_path, false);
    visVisitedNode(visited_nodes);
    pubGridPath2(full_path);
    // pubGridTwist(grid_twist);
    // pubGridTwist2(grid_twist);
    pubGridTwist3(full_path,vel_,acc_);
...
//sub
    ...
    _pts_sub   = nh.subscribe( "waypoints", 5, rcvWaypointsCallback );
    ...
    _rrpos_sub = nh.subscribe( "real_pose",1,realPoseCallback);
//pub
    ...
    nh.param("planning/threshold",threshold,     0.3);
