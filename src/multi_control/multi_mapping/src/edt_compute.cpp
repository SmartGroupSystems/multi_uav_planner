...
void EDTEnvironment::setMap(shared_ptr<GridMap> map) {
  this->grid_map_ = map;
  resolution_inv_ = 1 /grid_map_->getResolution();
}
...
void EDTEnvironment::getSurroundDistance(Eigen::Vector3d pts[2][2][2], double dists[2][2][2]) {
  for (int x = 0; x < 2; x++) {
    for (int y = 0; y < 2; y++) {
      for (int z = 0; z < 2; z++) {
        dists[x][y][z] = grid_map_->getDistance(pts[x][y][z]);
      }
    }
  }
}
...
pair<double, Eigen::Vector3d> EDTEnvironment::evaluateEDTWithGrad(const Eigen::Vector3d& pos,
  ...
  grid_map_->getSurroundPts(pos, sur_pts, diff);
...
double EDTEnvironment::evaluateCoarseEDT(Eigen::Vector3d& pos, double time) {
  double d1 =grid_map_->getDistance(pos);
  if (time < 0.0) {
    return d1;
  } else {
