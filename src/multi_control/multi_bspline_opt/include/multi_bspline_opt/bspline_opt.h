...
#include<multi_mapping/edt_compute.h>
using namespace std;
//宏定义
  ...
  typedef std::vector<OneTrajDataOfSwarm> SwarmTrajData;
    class bspline_optimizer
    {
        public:
           ...
           #define INIT_min_ellip_dist_ 123456789.0123456789
            ...
            double origin_x_, origin_y_,origin_z_;//地图起始点x,y，右手坐标系 ，最左下角的栅格子的中心
            double startPoint_x,startPoint_y,startPoint_z;//找到地图左上角的点，x负，y正
            ...
            Eigen::Vector3d drone_pos_world; //无人机位置
        ...
        public://函数
            bspline_optimizer() {}
            bspline_optimizer(const std::vector<Eigen::Vector3d> &path, const int&Dim,  const int&p,const Eigen::Vector3d drone_pos_world_, const Eigen::Vector4d other_drone_pose_);
            ...
            void setOptParam(const double lambda1,const double lambda2,const double lambda3,const double lambda4,const double lambda5,
                                                    const double safe_dist,const double swarm_clearance_,const double planning_horizen);
            void setEnvironment(const EDTEnvironment::Ptr& env) ;                                       
           void setMapParam(const double &origin_x,const double &origin_y,const double &origin_z, const double &map_resolution,
            ...
            inline void setPath(const std::vector<Eigen::Vector3d> &path)
            {
                path_.clear();
                path_ = path;
                cps_num_ = path.size() + 2*p_order_ -2;
            }
            void setPosition(const Eigen::Vector3d drone_pos_world_, const Eigen::Vector4d other_drone_pose_);
            //nlopt相关
    ...
    class plan_manager
    {
        public:
            ...
            double start_x_, start_y_,start_z_;// A star找到的起点
            double goal_x_,goal_y_,goal_z_;//A star找到的终点
            int map_size_x,map_size_y,map_size_z, map_size;//esdf x y
        ...
        public:
            ...
            bool  astar_subCallback(const std::vector<Eigen::Vector3d> &astar_path_);
    ...
    //         Eigen::Vector3i posToIndex(const Eigen::MatrixXd &pos)
    //         {
    //             Eigen::Vector3i curr_index;
